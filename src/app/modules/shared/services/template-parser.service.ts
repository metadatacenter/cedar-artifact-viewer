...
export class TemplateParserService {
  constructor(
    @Inject('instance') @Optional() public instance?: any,
    @Inject('template') @Optional() public template?: any,
  ) {
  ...
  static attributeValueNode(
    ...
    const nodeType = TemplateParserService.getNodeType(
      schema,
      InputType.attributeValue,
    );
    const nodeSubtype = TemplateParserService.getNodeSubtype(
      InputType.attributeValue,
    );
    return {
      key: key,
      ...
      valueLocation: TemplateService.getValueLocation(
        schema,
        nodeType,
        nodeSubtype,
      ),
        ...
        if (model !== null && model !== undefined) {
          if (!Object.hasOwn(model, key)) {
            ...
            if (TemplateService.isElement(schema)) {
              model[key] = TemplateService.initValue(
                schema,
                key,
                InputType.element,
                minItems,
          ...
          if (TemplateService.isElement(schema)) {
            ...
            const node = TemplateParserService.staticNode(
              schema,
              model,
              type,
              minItems,
            ...
            if (InputTypeService.isAttributeValue(type)) {
              ...
              const node = TemplateParserService.attributeValueNode(
                schema,
                model,
                key,
                items,
