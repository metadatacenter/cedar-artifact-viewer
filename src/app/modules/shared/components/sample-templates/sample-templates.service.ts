...
export class SampleTemplatesService {
  ...
  loadTemplate(templateLocationPrefix: string, templateNum: string): void {
    ...
    const templateUrl =
      templateLocationPrefix + templateNum + '/' + this.TEMPLATE_FILENAME;
    this.loadTemplateFromURL(templateUrl, templateNum);
    const metadataUrl =
      templateLocationPrefix + templateNum + '/' + this.METADATA_FILENAME;
    ...
    this.http.get(templateUrl).subscribe(
      ...
      (error) => {
        ...
        this.messageHandlerService.error(
          'Error while loading sample template from: ' + templateUrl,
        );
        this.handleLoadedDataFiles();
      },
    );
  ...
  loadMetadataFromURL(metadataUrl: string, templateNum: string = null): void {
    ...
    this.http.get(metadataUrl).subscribe(
      ...
      (error) => {
        ...
        this.loadedMetadata = null;
        this.messageHandlerService.error(
          'Error while loading sample metadata from: ' + metadataUrl,
        );
        this.handleLoadedDataFiles();
      },
    );
  ...
  getSampleTemplatesFromRegistry(
    ...
    templateLocationPrefix = this.fixedLocationPrefix(templateLocationPrefix);
    const registryUrl =
      templateLocationPrefix + this.TEMPLATE_REGISTRY_FILENAME;
  ...
  private getAllTemplatesSubscription(
    templateLocationPrefix: string,
  ): Observable<object> {
    ...
    for (let i = 1; i <= this.MAX_CHECK; i++) {
      ...
      const templateUrl =
        templateLocationPrefix + templateName + '/' + this.TEMPLATE_FILENAME;
      ...
      concatMap((singleUrl) => {
        ...
        return this.getSingleTemplateLabel(singleUrl).pipe(
          map((templateLabel) => {
            errorIndex = 0;
            const templateEntry = {};
            templateEntry[templateNum] =
              'Template ' + templateNum + ' - ' + templateLabel;
