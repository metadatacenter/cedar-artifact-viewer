...
export class QuestionComponent implements OnInit, OnChanges {
    ...
    switch (this.node.type) {
      ...
      case InputType.controlled:
        this.formGroup = this.fb.group({
          values: this.fb.array(this.buildControlled(this.node, false)),
        });
      ...
      case InputType.temporal:
        this.formGroup = this.fb.group({
          values: this.fb.array(
            this.allowMultipleControls(this.node, false, validators),
          ),
        });
      ...
      case InputType.checkbox:
        this.formGroup = this.fb.group({
          values: this.fb.array(this.allowMultipleOptions(this.node, false)),
        });
        this.parentGroup.setControl(this.node.key, this.formGroup);
        this.setDisable(this.formGroup, this.mode === 'view');
        break;
      ...
      case InputType.attributeValue:
        this.formGroup = this.fb.group({
          values: this.fb.array(this.buildAV(this.node, false)),
        });
        this.parentGroup.setControl(this.node.key, this.formGroup);
        this.setDisable(this.formGroup, this.mode === 'view');
        break;
  ...
  onSelectedControlled(event) {
    InstanceService.addControlledValue(
      this.node.model,
      this.node.key,
      event['@id'],
      event['prefLabel'],
  ...
  onRemovedControlled(index) {
    InstanceService.removeControlledValue(
      this.node.model,
      this.node.key,
      index,
    );
  ...
  get isValid() {
    ...
    if (
      this.parentGroup &&
      Object.hasOwn(this.parentGroup.controls, this.node.key)
    ) {
    ...
    switch (this.node.type) {
      case InputType.controlled:
        ...
        this.formGroup.setControl(
          'values',
          this.fb.array(this.buildControlled(node, false)),
        );
        break;
      ...
      case InputType.temporal:
        ...
        this.formGroup.setControl(
          'values',
          this.fb.array(this.allowMultipleControls(node, false, validators)),
        );
        // this.formGroup.updateValueAndValidity({onlySelf: true, emitEvent: true});
        break;
    }
  ...
  private buildControlled(node: TreeNode, disabled: boolean): any[] {
    ...
    if (node.model[node.key]) {
      if (Array.isArray(node.model[node.key])) {
        ...
        if (Object.hasOwn(node.model[node.key], 'rdfs:label')) {
  ...
  openControlledSource(node: TreeNode) {
    ...
    if (node && node.valueConstraints) {
      ...
      if (vc.classes.length > 0) {
        ...
        urlSuffix =
          encodeURIComponent(classInfo.source) +
          '?p=classes&conceptid=' +
        ...
        urlSuffix =
          encodeURIComponent(branchInfo.acronym) +
          '?p=classes&conceptid=' +
          encodeURIComponent(branchInfo.uri);
      } else if (vc.valueSets.length > 0) {
        const valueSetInfo = vc.valueSets[0];
        urlSuffix =
          encodeURIComponent(valueSetInfo.vsCollection) +
          '?p=classes&conceptid=' +
          encodeURIComponent(valueSetInfo.uri);
      }
      if (urlSuffix !== null) {
        window.open(
          'https://bioportal.bioontology.org/ontologies/' + urlSuffix,
          '_blank',
        );
      }
    }
